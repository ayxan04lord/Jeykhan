...
class ServiceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("title", "category", "description", "icon")}),
        (
            "Display Settings",
...
class ContactInfoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Contact Details", {"fields": ("phone_number", "email", "address")}),
        ("Settings", {"fields": ("is_active",), "classes": ("collapse",)}),
    )
    ...
    def has_add_permission(self, request):
        # Limit to one active contact info at a time
